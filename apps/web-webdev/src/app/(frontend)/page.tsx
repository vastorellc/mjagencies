...
const WHY_US: { title: string; description: string }[] = [
  {
    ...
    description:
      'Everything is production-code, not drag-and-drop. Your codebase is an asset, not a liability.',
  },
  {
    title: 'Performance designed in',
    description:
      'We target Core Web Vitals from architecture, not from a Lighthouse audit two weeks before launch.',
  },
  {
    title: 'Accessibility as standard',
    description:
      'WCAG 2.2 AA compliance tested in CI with axe-core. Accessibility is in the spec, not the punch list.',
  },
]

export default async function HomePage(): Promise<React.ReactElement> {
  const page = await fetchPageBySlug(AGENCY_ID, 'home')
  const headline = page?.title ?? 'Websites and web apps built to production standards'
  const subheadline =
    page?.aio_tldr ??
    'We build custom sites, headless CMS platforms, and web applications in Next.js and TypeScript — fast, accessible, and maintainable.'
