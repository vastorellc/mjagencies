...
const AGENCY_ID = 'fitness'

export const revalidate = 60

export const metadata: Metadata = {
  title: 'MJ Fitness Agency — Marketing that drives results',
  description: 'Fitness marketing — gym, studio, and trainer growth through local SEO, paid ads, and member retention systems.',
}

export default async function HomePage(): Promise<React.ReactElement> {
  const page = await fetchPageBySlug(AGENCY_ID, 'home')
  const headline = page?.title ?? 'MJ Fitness Agency'
  const subheadline = page?.aio_tldr ?? 'Fitness marketing — gym, studio, and trainer growth through local SEO, paid ads, and member retention systems.'
