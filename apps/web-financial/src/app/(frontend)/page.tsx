...
const AGENCY_ID = 'financial'

export const revalidate = 60

export const metadata: Metadata = {
  title: 'MJ Financial Agency — Marketing that drives results',
  description: 'Financial services marketing — RIA growth, advisor lead generation, and compliant content for the financial industry.',
}

export default async function HomePage(): Promise<React.ReactElement> {
  const page = await fetchPageBySlug(AGENCY_ID, 'home')
  const headline = page?.title ?? 'MJ Financial Agency'
  const subheadline = page?.aio_tldr ?? 'Financial services marketing — RIA growth, advisor lead generation, and compliant content for the financial industry.'
