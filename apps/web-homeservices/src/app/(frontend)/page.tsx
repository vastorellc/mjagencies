...
const AGENCY_ID = 'homeservices'

export const revalidate = 60

export const metadata: Metadata = {
  title: 'MJ Home Services Agency — Marketing that drives results',
  description: 'Home services marketing — HVAC, plumbing, electrical, and landscaping growth via Local Services Ads and reputation.',
}

export default async function HomePage(): Promise<React.ReactElement> {
  const page = await fetchPageBySlug(AGENCY_ID, 'home')
  const headline = page?.title ?? 'MJ Home Services Agency'
  const subheadline = page?.aio_tldr ?? 'Home services marketing — HVAC, plumbing, electrical, and landscaping growth via Local Services Ads and reputation.'
