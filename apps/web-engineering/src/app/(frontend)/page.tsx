...
export const metadata: Metadata = {
  title: 'MJ Engineering Agency — Software that ships on time and stays maintainable',
  description:
...
const WHY_US: { title: string; description: string }[] = [
  {
    title: 'Technical debt as a first-class concern',
    description:
      'We assess the codebase before quoting new features. We will not build on a foundation that will collapse.',
  },
  {
    title: '2-week sprints, weekly demos',
    description:
      'Every sprint ships something usable. You always know what your budget built last week.',
  },
  {
    title: 'ADR documentation',
    description:
      'Architecture decisions are documented as ADRs in the repo. Future engineers understand why — not just what.',
  },
]

export default async function HomePage(): Promise<React.ReactElement> {
  const page = await fetchPageBySlug(AGENCY_ID, 'home')
  const headline = page?.title ?? 'Software that ships on time and stays maintainable'
  const subheadline =
    page?.aio_tldr ??
    'We build full-stack applications, cloud infrastructure, and DevOps pipelines for product teams that need to move fast without accumulating debt.'
