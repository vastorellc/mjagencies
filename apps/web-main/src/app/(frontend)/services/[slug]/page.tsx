...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!page) return {}
  return { title: page.meta_title ?? `${page.title} — MJAgency`, description: page.meta_description ?? page.aio_tldr }
}
...
export default async function ServicePage({ params }: { params: Promise<{ slug: string }> }): Promise<React.ReactElement> {
  ...
  const jsonLd = {
    ...
    '@type': 'Service',
    name: page.title,
    provider: { '@type': 'Organization', name: 'MJAgency' },
    areaServed: 'US',
  }
