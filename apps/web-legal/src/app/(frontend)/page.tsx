...
export const metadata: Metadata = {
  title: 'MJ Legal Agency — Marketing that drives results',
  description: 'Law firm marketing — case generation, ethical PPC, content marketing, and Bar-compliant client acquisition.',
}

export default async function HomePage(): Promise<React.ReactElement> {
  const page = await fetchPageBySlug(AGENCY_ID, 'home')
  const headline = page?.title ?? 'MJ Legal Agency'
  const subheadline = page?.aio_tldr ?? 'Law firm marketing — case generation, ethical PPC, content marketing, and Bar-compliant client acquisition.'
