...
const AGENCY_ID = 'realestate'

export const revalidate = 60

export const metadata: Metadata = {
  title: 'MJ Real Estate Agency — Marketing that drives results',
  description: 'Real estate marketing — agent and brokerage growth via lead gen, listing presentation, and territory dominance.',
}

export default async function HomePage(): Promise<React.ReactElement> {
  const page = await fetchPageBySlug(AGENCY_ID, 'home')
  const headline = page?.title ?? 'MJ Real Estate Agency'
  const subheadline = page?.aio_tldr ?? 'Real estate marketing — agent and brokerage growth via lead gen, listing presentation, and territory dominance.'
