...
export const metadata: Metadata = {
  title: 'Ecommerce Insights — MJ Ecommerce Agency Blog',
  description: 'Practical ecommerce growth strategies, Shopify tutorials, CRO case studies, and paid acquisition breakdowns from the MJ Ecommerce Agency team.',
}
  ...
  return (
    <>
      ...
      <main id="main-content" style={{ padding: 'var(--mj-space-16) var(--mj-space-6)' }}>
        ...
        <p style={{ fontSize: 'var(--mj-text-size-xl)', color: 'var(--mj-color-text-secondary)', marginTop: 'var(--mj-space-4)' }}>
          Growth strategies, CRO tactics, and Shopify deep-dives from our team.
        </p>
        {posts.length === 0 ? (
          <p style={{ marginTop: 'var(--mj-space-16)', color: 'var(--mj-color-text-secondary)', fontSize: 'var(--mj-text-size-lg)' }}>
            No posts published yet. Check back soon.
          </p>
        ) : (
          <ul style={{ marginTop: 'var(--mj-space-12)', display: 'grid', gap: 'var(--mj-space-8)', listStyle: 'none', padding: 0 }} aria-label="Blog posts">
            {posts.map(post => (
              <li key={post.id}>
                <article>
                  {post.featured_image && (
                  ...
                  {post.publishedAt && (
                    <time
                      ...
                      style={{ fontSize: 'var(--mj-text-size-sm)', color: 'var(--mj-color-text-tertiary)', marginTop: 'var(--mj-space-2)', display: 'block' }}
                    >
                      {new Date(post.publishedAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}
                    </time>
                  )}
                  {post.excerpt && (
                    <p style={{ color: 'var(--mj-color-text-secondary)', marginTop: 'var(--mj-space-3)', maxWidth: '65ch' }}>
                      {post.excerpt}
                    </p>
                  )}
                  <a
