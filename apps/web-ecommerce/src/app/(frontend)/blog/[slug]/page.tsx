...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!post) return {}
  return {
    title: `${post.title} — MJ Ecommerce Agency`,
    description: post.excerpt ?? post.aio_tldr,
  }
}
  ...
  return (
    <>
      ...
      <main id="main-content" style={{ padding: 'var(--mj-space-16) var(--mj-space-6)', maxWidth: '75ch', margin: '0 auto' }}>
        ...
        <article>
          <h1 style={{ fontSize: 'var(--mj-text-size-4xl)', fontWeight: 'var(--mj-weight-bold)', color: 'var(--mj-color-text-primary)', fontFamily: 'var(--mj-font-heading)', lineHeight: 'var(--mj-leading-tight)' }}>
            {post.title}
          </h1>
          {post.publishedAt && (
            <time
              dateTime={post.publishedAt}
              style={{ fontSize: 'var(--mj-text-size-sm)', color: 'var(--mj-color-text-tertiary)', marginTop: 'var(--mj-space-4)', display: 'block' }}
            >
          ...
          {post.featured_image && (
            <MjImage
              ...
              blurHash={post.featured_image.blurhash ? { hash: post.featured_image.blurhash, width: post.featured_image.width, height: post.featured_image.height } : undefined}
              priority
          ...
          {post.excerpt && (
            <p style={{ fontSize: 'var(--mj-text-size-xl)', color: 'var(--mj-color-text-secondary)', marginTop: 'var(--mj-space-8)', fontStyle: 'italic' }}>
              {post.excerpt}
            </p>
          )}
          {post.author && (
            <section aria-label="Article author" style={{ marginTop: 'var(--mj-space-12)', paddingTop: 'var(--mj-space-8)', borderTop: '1px solid var(--mj-color-border)' }}>
              ...
              {post.author.bio && (
                <p style={{ color: 'var(--mj-color-text-secondary)', marginTop: 'var(--mj-space-2)', fontSize: 'var(--mj-text-size-sm)' }}>{post.author.bio}</p>
              )}
            </section>
          )}
        </article>
        <nav aria-label="Blog navigation" style={{ marginTop: 'var(--mj-space-12)' }}>
          <a
            href="/blog"
            style={{ color: 'var(--mj-color-brand-500)', fontWeight: 'var(--mj-weight-medium)', textDecoration: 'none' }}
          >
