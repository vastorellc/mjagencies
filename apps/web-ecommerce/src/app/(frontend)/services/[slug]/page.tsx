...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!page) return {}
  return {
    title: page.meta_title ?? `${page.title} — MJ Ecommerce Agency`,
    description: page.meta_description ?? page.aio_tldr,
  }
}
...
export default async function ServicePage({ params }: { params: Promise<{ slug: string }> }): Promise<React.ReactElement> {
  ...
  const serviceJsonLd = {
    ...
    '@type': 'Service',
    name: page.title,
    description: page.aio_tldr ?? page.meta_description ?? '',
    provider: {
      '@type': 'Organization',
      name: 'MJ Ecommerce Agency',
      url: 'https://ecommerce.mjagency.com',
    },
    areaServed: 'US',
  }
  ...
  return (
    <>
      ...
      <main id="main-content" style={{ padding: 'var(--mj-space-16) var(--mj-space-6)' }}>
        ...
        <h1 style={{ fontSize: 'var(--mj-text-size-4xl)', fontWeight: 'var(--mj-weight-bold)', color: 'var(--mj-color-text-primary)', fontFamily: 'var(--mj-font-heading)', marginTop: 'var(--mj-space-8)' }}>
          {page.title}
        </h1>
        {page.aio_tldr && (
        ...
        <div style={{ display: 'flex', gap: 'var(--mj-space-4)', marginTop: 'var(--mj-space-10)', flexWrap: 'wrap' }}>
          <a
            href="/contact"
