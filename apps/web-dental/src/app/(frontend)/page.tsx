...
export const metadata: Metadata = {
  title: 'MJ Dental Agency — Marketing that drives results',
  description: 'Dental practice marketing — patient acquisition, schedule filling, and growth for solo and group practices.',
}

export default async function HomePage(): Promise<React.ReactElement> {
  const page = await fetchPageBySlug(AGENCY_ID, 'home')
  const headline = page?.title ?? 'MJ Dental Agency'
  const subheadline = page?.aio_tldr ?? 'Dental practice marketing — patient acquisition, schedule filling, and growth for solo and group practices.'
