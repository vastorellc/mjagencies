...
- [ ] `backend/vitest.config.ts` — test framework config; `npm install -D vitest` in backend
- [ ] `backend/tests/auth.test.ts` — stubs for AUTH-02 (401 without token, 200 with valid token)
- [ ] `backend/tests/rls.test.ts` — stubs for AUTH-04 (cross-user data isolation via RLS)
- [ ] `backend/tests/admin.test.ts` — stubs for AUTH-05 (non-admin 403), AUTH-06 (admin 200)
- [ ] `backend/tests/storage.test.ts` — stubs for STORE-01 (uploads dir created on startup)
...
| `/var/uploads/test.txt` served via Nginx | STORE-02 | Requires live VPS + Nginx | Create test file → `curl VPS_PUBLIC_URL/uploads/test.txt` → 200 with content |
