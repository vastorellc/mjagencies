...
| `_seed_state` | No | No | No | No | 02-04 |
| `mfa_config` | Yes | Yes (005) | Yes | Yes (005) | 03-02 |

## Migration Apply Order

Migrations are applied by Plan 02-03's runner in this exact order per agency DB:

1. `src/migrations/0000_initial.sql` — drizzle-kit generated; creates initial tables + RLS ENABLE + policies
2. `src/migrations/0001_mfa_config.sql` — Plan 03-02; adds mfa_config table + RLS + policy + unique index
3. `src/migrations/custom/001_agency_id_immutable.sql` — `prevent_agency_id_change()` trigger on all agency-scoped tables
4. `src/migrations/custom/002_force_rls_and_app_role.sql` — `FORCE ROW LEVEL SECURITY` + `REVOKE UPDATE/DELETE ON audit_log` + per-agency DML grants
...
| Table | Plan | RLS | FORCE RLS | Audit | Description |
|-------|------|-----|-----------|-------|-------------|
| `mfa_config` | 03-02 | YES | YES (005) | YES (005) | MFA configuration per (agencyId, userId): recovery hashes + lockout columns |
...
| M003 (MFA Schema) | DELIVERED in Plan 03-02 | mfa_config table, RLS + FORCE RLS + audit trigger, 005 custom migration |
