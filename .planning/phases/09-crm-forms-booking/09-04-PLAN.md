...
### Task 3: Email DNS setup wizard admin page (SC-4)

**Files:**
- `apps/web-main/src/app/(payload)/admin/email-setup/page.tsx`

**Acceptance criteria:**
- [ ] Page calls `validateEmailDns(domain)` server-side and renders pass/fail status for DKIM, SPF, and DMARC
- [ ] Returns `validateDkim`, `validateSpf`, `validateDmarc` results from `@mjagency/email`
- [ ] Accessible only to authenticated admin/super_admin users (requireSession check)
- [ ] Shows `{ valid: boolean, record?: string }` for each check with a green/red indicator
- [ ] Domain input accepted via search param `?domain=yourdomain.com`

<action>
**Step 1 — Create apps/web-main/src/app/(payload)/admin/email-setup/page.tsx.**
...
export default async function EmailSetupPage({ searchParams }: PageProps): Promise<React.ReactElement> {
  ...
  return (
    <main style={{ padding: 'var(--mj-space-8)', maxWidth: '640px' }}>
      <h1 style={{ fontSize: 'var(--mj-text-size-2xl)', fontWeight: 'var(--mj-weight-bold)', color: 'var(--mj-color-text-primary)' }}>
        Email DNS Setup
      </h1>
      <form method="get" style={{ marginTop: 'var(--mj-space-6)', display: 'flex', gap: 'var(--mj-space-3)' }}>
        <input
          name="domain"
          type="text"
          ...
          style={{ flex: 1, padding: 'var(--mj-space-2) var(--mj-space-3)', borderRadius: 'var(--mj-radius-md)', border: '1px solid var(--mj-color-border)', fontSize: 'var(--mj-text-size-base)' }}
        />
        <button
          type="submit"
          style={{ padding: 'var(--mj-space-2) var(--mj-space-4)', backgroundColor: 'var(--mj-color-brand-500)', color: '#fff', borderRadius: 'var(--mj-radius-md)', border: 'none', fontWeight: 'var(--mj-weight-medium)', cursor: 'pointer' }}
        >
          Check
        </button>
      </form>
      ...
      {cleanDomain && (
        <ul style={{ marginTop: 'var(--mj-space-8)', listStyle: 'none', padding: 0, display: 'flex', flexDirection: 'column', gap: 'var(--mj-space-4)' }}>
          {checks.map(({ label, result }) => (
            ...
            >
              <strong style={{ color: result?.valid ? 'var(--mj-color-success)' : 'var(--mj-color-error)' }}>
                {result?.valid ? '✓' : '✗'} {label}
              </strong>
              {result?.record && (
                <code style={{ display: 'block', marginTop: 'var(--mj-space-2)', fontSize: 'var(--mj-text-size-sm)', wordBreak: 'break-all', color: 'var(--mj-color-text-secondary)' }}>
                  {result.record}
                </code>
              )}
              {!result?.valid && !result?.record && (
                <p style={{ marginTop: 'var(--mj-space-2)', color: 'var(--mj-color-text-secondary)', fontSize: 'var(--mj-text-size-sm)' }}>
...
**Verification:**
```bash
pnpm typecheck --filter=@mjagency/web-main
```
Exits 0.
</action>

## Commit message
feat(09-04): email engine — SMTP sender, encrypted queue/worker, DNS validation, 35-day warmup gate, sequences + admin DNS wizard
