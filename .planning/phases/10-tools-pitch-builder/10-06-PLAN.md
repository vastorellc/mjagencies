...
goal: "Build the complete invoicing system: Payload invoices collection with 6-state machine, Stripe + PayPal checkout links, dunning sequences (3 reminders + 30-day close), partial payment tracker, refund flow, and automatic chargeback evidence package compilation from proposal + e-sign records."
...
must_haves:
  truths:
    - "Invoice status machine has exactly 6 states: draft → sent → viewed → paid → refunded → disputed"
    ...
    - "Chargeback evidence package compiled from: proposal body_json + esign r2_key + email delivery logs"
  ...
  artifacts:
    - path: "packages/invoices/src/collections/invoices.ts"
      provides: "Payload invoices collection with 6-state machine"
...
Purpose: REQ-127 (deposit auto-on-sign), REQ-128 (partial payment tracker), REQ-129 (refund + chargeback), REQ-418 (6-state machine), REQ-419 (evidence auto-compiled).
...
/**
 * packages/db/src/schema/invoices.ts
 * REQ-418: 6-state machine (draft→sent→viewed→paid→refunded→disputed).
...
/**
 * packages/invoices/src/collections/invoices.ts
 * REQ-418: invoice 6-state machine.
  ...
  createEncryptedWorker<StripeInvoiceEventJobData>(
    ...
    async (job) => {
      ...
      if (event.type === 'checkout.session.completed') {
        ...
        const { proposalId, esignId } = session.metadata ?? {}
      ...
      if (event.type === 'charge.dispute.created') {
        ...
        // Find invoice by charge ID (look up payment intent → invoice)
        // REQ-419: compile chargeback evidence
        const chargebackEvidence = {
          compiledAt: new Date().toISOString(),
          disputeId: dispute.id,
          chargeId: dispute.charge,
          note: 'This package was automatically compiled from your proposal, e-signed contract, and delivery records.',
          proposalLink: `/admin/collections/proposals?where[esign_id][exists]=true`,
          esignLink: `/admin/collections/esign_records`,
        }

        // Update invoice to disputed status + attach evidence
        const disputeSearchRes = await fetch(
          `${PAYLOAD_URL}/api/invoices?where[agency_id][equals]=${agencyId}&where[status][in]=paid,partial&limit=50`,
          { headers: { Authorization: `Bearer ${PAYLOAD_API_KEY}` } },
        )
        const disputeData = await disputeSearchRes.json() as { docs: Array<{ id: string }> }
        for (const inv of disputeData.docs) {
          await fetch(`${PAYLOAD_URL}/api/invoices/${inv.id}`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${PAYLOAD_API_KEY}` },
            body: JSON.stringify({
              status: 'disputed',
          ...
          log.warn({ invoiceId: inv.id, disputeId: dispute.id }, 'Invoice marked disputed, chargeback evidence compiled')
          break // Only update first match — improve with exact charge match in production
        }
      }
    },
...
| Chargeback evidence package → dispute resolution | Package includes proposal + esign links |
...
<success_criteria>
- Invoice state machine has exactly 6 states (including partial) — all correctly named per UI-SPEC
...
- Chargeback evidence compiled automatically on charge.dispute.created event
</success_criteria>
